...
class UsuariosSetores extends Model
{ 
    private $tabela = 'tb_usuarios_setores';
    private $Model = '';
    ...
    public function checarUsuarioSetor($ver = 0)
    {
        $parametros = "WHERE USU_COD='{$this->codUsuario}' AND EMP_COD='{$this->codEmpresa}' AND SET_COD='{$this->codigo}'";
        $campos = "*";
        $resultado = $this->Model->exibir($parametros, $campos, $ver, $id = false);
        if ($resultado) {
            //Já existe
