...
class produtos extends View
{
        ...
        if (isset($dados[2]) && $dados[2] != '' && isset($dados[3]) && $dados[3] != '') {
            ...
            if (isset($_SESSION['EMP_COD']) && $_SESSION['EMP_COD'] == $dados[2]){
                ...
                if ($this->dados['estoque'] != 0) {
                    //$this->link[2] = ['link'=> 'estoques/'.$dados[2],'nome' => 'GERENCIAR ESTOQUE'];
                    $this->link[2] = ['link'=> 'estoques/produtos/'.$dados[2].'/'.$dados[3],'nome' => 'GERENCIAR PRODUTOS'];
                    $this->link[3] = ['link'=> 'produtos/cadastro/'.$dados[2].'/'.$dados[3],'nome' => 'CADASTRAR PRODUTOS'];    
                    $ok = true;
                }
            }else {
        ...
        if (isset($_POST) && isset($dados['CADASTRAR_NOVO_PRODUTO'])) {
            ...
            if($_SESSION['USU_COD'] == $dados['USU_COD'] && isset($_SESSION['EMP_COD']) && $_SESSION['EMP_COD'] == $dados['EMP_COD']){
                ...
                if (!$pro) {
                    ...
                    $this->link[2] = ['link'=> 'estoques/produtos/'.$_SESSION['EMP_COD'].'/'.$dados['EST_COD'],'nome' => 'GERENCIAR PRODUTOS'];
                    $this->link[3] = ['link'=> 'produtos/cadastro/'.$_SESSION['EMP_COD'].'/'.$dados['EST_COD'],'nome' => 'CADASTRAR PRODUTOS'];
    ...
    {
        $this->dados['title'] .= ' DETALHAR PRODUTO';
        
        $this->dados['breadcrumb'] = $this->Check->setLink($this->link)->breadcrumb();
        $dados = filter_input_array(INPUT_GET, FILTER_SANITIZE_URL);
        $dados = explode("/",$dados['url']);
        $ok = false;
        if (isset($dados[1]) && $dados[1] == 'detalhar' && isset($dados[2]) && isset($dados[3]) && isset($dados[4])) {
            ...
            if($this->dados['empresa']['USU_COD'] == $_SESSION['USU_COD'] && $this->dados['empresa']['EMP_COD'] == $dados[2]){
                ...
                $this->dados['produto'] = $this->Produtos->setCodEmpresa($dados[2])->setCodEstoque($dados[3])->setCodigo($dados[4])->listar(0);
                if ($this->dados['produto'] != 0) {
                    $this->dados['movimentacoes_produto_entradas'] = $this->Movimentacoes->setCodEmpresa($dados[2])->setCodEstoque($dados[3])->setCodProduto($dados[4])->setTipo(1)->setStatus(1)->listarTodasMovimentacoesProdutos(0);
                    $this->dados['movimentacoes_produto_saidas'] = $this->Movimentacoes->setCodEmpresa($dados[2])->setCodEstoque($dados[3])->setCodProduto($dados[4])->setTipo(2)->setStatus(1)->listarTodasMovimentacoesProdutos(0);
        ...
        $this->dados['breadcrumb'] = $this->Check->setLink($this->link)->breadcrumb();
        if($ok){
            $this->render('admin/estoques/produtos/detalhar', $this->dados);
        }else{
            $this->dados['produtos'] = $this->Produtos->setCodEmpresa($_SESSION['EMP_COD'])->listarTodos(0);
            $this->render('admin/estoques/produtos/listar', $this->dados);
        }
